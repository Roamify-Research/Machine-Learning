...
def search_user_ratings(user, attraction):
    ...
    if attraction in user_ratings_data["Attraction"].values:
        rating = user_ratings_data.loc[
            user_ratings_data["Attraction"] == attraction, user
        ].values[0]
        return rating
    else:
        return 0
...
def load_data():
    attractions_data = pd.read_csv(
        ...
        usecols=["Name", "State", "City", "Opening Hours", "Description", "Rating"],
    )
    user_ratings_data = pd.read_csv("../datasets/user_attractions_ratings.csv")
...
def load_user_data(user):
    ...
    user_ratings_data = pd.read_csv(
        "../datasets/predicted_user_ratings.csv", usecols=["Attraction", user]
    )
