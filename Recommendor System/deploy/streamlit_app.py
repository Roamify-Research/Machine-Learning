...
st.markdown(
    "<h3 style='text-align: center;'>Adding User Ratings</h3>", unsafe_allow_html=True
)
...
if new_user != "":
    ...
    available_attractions = attractions_data[attractions_data["State"] == state_][
        "Name"
    ].tolist()
    with st.form(key="ratings_form"):
        st.write("Rate the attractions you have visited:")
        for attraction in available_attractions:
            existing_rating = attractions_data.loc[
                attractions_data["Name"] == attraction, new_user
            ].values[0]
            ...
            rating = st.slider(
                ...
                min_value=0,
                max_value=5,
                value=int(existing_rating),
                step=1,
                key=f"rating_{attraction}",
            )
            if rating != existing_rating:
                attractions_data.loc[
                    attractions_data["Name"] == attraction, new_user
                ] = rating
    ...
    if submit_button:
        ...
        worksheet.clear()
        worksheet.update(
            [attractions_data.columns.values.tolist()]
            + attractions_data.values.tolist()
        )
